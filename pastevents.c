...
History history;
bool PasteventsExecuteFlag = false;
char *PasteventsExecuteString = NULL;
...
void pastevents(char **args, char *permenant_home)
{
    ...
    if (strcmp(args[0], "execute") == 0)
    {
        ...
        PasteventsExecuteFlag = true;
        if (PasteventsExecuteString != NULL)
        {
            strcat(PasteventsExecuteString, ";");
            strcat(PasteventsExecuteString, command_name);
        }
        else
        {
            PasteventsExecuteString = (char *)malloc(sizeof(char) * MAX_COMMAND_LENGTH);
            strcpy(PasteventsExecuteString, command_name);
        }
